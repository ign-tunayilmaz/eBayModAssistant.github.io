...
const ModerationTool = () => {
  ...
  // Advanced Webscraping v2.2 state
...
Analyze this eBay community post and provide a moderation recommendation. Consider the priority level and any patterns from previous moderation decisions.`
  ...
  // Advanced Webscraping v2.2 Functions
  const scrapeEbayPost = async (url, useAI = true, forceRefresh = false) => {
    if (!url?.trim()) {
      setScrapingError('Please enter a valid eBay community URL');
      return;
    }
    ...
    try {
      ...
      const response = await fetch('/api/scrape/ebay-post', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      if (result.success) {
        ...
        setScrapingHistory(prev => [historyEntry, ...prev.slice(0, 19)]); // Keep last 20
        
        console.log(`✅ Scraping successful: ${result.data.username} - "${result.data.title}"`);
        console.log(`📊 Method: ${result.method}, AI Parsed: ${result.aiParsed}, Cached: ${result.cached}`);
        
      } else {
        setScrapingError(result.error || 'Scraping failed');
        console.error('❌ Scraping failed:', result.error);
      }

    } catch (error) {
      console.error('❌ Scraping request failed:', error);
      setScrapingError(`Request failed: ${error.message}`);
    } finally {
      setIsScrapingLoading(false);
    }
  };

  const batchScrapeEbayPosts = async () => {
    ...
    if (urls.length > 10) {
      alert('Maximum 10 URLs per batch request');
      return;
    }
    ...
    try {
      ...
      console.log(`✅ Batch scraping completed: ${result.successfulScrapes}/${result.totalUrls} successful`);
      
      if (result.errors.length > 0) {
        console.warn('❌ Some URLs failed:', result.errors);
      }

    } catch (error) {
      console.error('❌ Batch scraping failed:', error);
      setScrapingError(`Batch scraping failed: ${error.message}`);
    } finally {
      setIsScrapingLoading(false);
    }
  };
  ...
  const applyScrapedDataToTemplate = (templateId, scrapedData) => {
    if (!scrapedData) {
      alert('No scraped data available to apply');
      return;
    }
  ...
  return h('div', { className: bgClass },
    h('div', { className: 'max-w-5xl mx-auto' },
      ...
      // Advanced Webscraping v2.2 Section
      h('div', { className: `${cardBg} rounded-lg shadow p-4 mb-6 border ${borderColor}` },
        h('button', { 
          onClick: () => setShowWebscraping(!showWebscraping),
          className: 'w-full flex items-center justify-between mb-3'
        },
          h('h3', { className: `font-semibold ${textPrimary}` }, '🕷️ Advanced Webscraping v2.2'),
          h('div', { className: 'flex items-center gap-2' },
            h('span', { className: `text-sm ${textSecondary}` }, showWebscraping ? 'Hide' : 'Show'),
            showWebscraping ? h(ChevronUp, { className: `w-5 h-5 ${textSecondary}` }) : h(ChevronDown, { className: `w-5 h-5 ${textSecondary}` })
          )
        ),
        showWebscraping && h('div', { className: 'space-y-4' },
          // Single URL scraping
          h('div', { className: `border rounded-lg p-4 ${borderColor}` },
            h('h4', { className: `font-semibold ${textPrimary} mb-3` }, 'Single Post Scraping'),
            h('div', { className: 'space-y-3' },
              h('div', { className: 'flex gap-2' },
                h('input', {
                  type: 'url',
                  placeholder: 'Enter eBay community post URL...',
                  value: scrapingUrl,
                  onChange: (e) => setScrapingUrl(e.target.value),
                  className: `flex-1 px-3 py-2 text-sm border rounded ${darkMode ? 'bg-slate-800 border-slate-700 text-slate-200 placeholder-slate-400' : 'bg-white border-slate-300'}`
                }),
                h('button', {
                  onClick: () => scrapeEbayPost(scrapingUrl, true, false),
                  disabled: isScrapingLoading || !scrapingUrl.trim(),
                  className: `px-4 py-2 text-sm font-medium rounded ${
                    isScrapingLoading || !scrapingUrl.trim() 
                      ? 'bg-gray-400 cursor-not-allowed text-white' 
                      : 'bg-green-600 hover:bg-green-700 text-white'
                  }`
                }, isScrapingLoading ? 'Scraping...' : '🕷️ Scrape'),
                h('button', {
                  onClick: () => scrapeEbayPost(scrapingUrl, true, true),
                  disabled: isScrapingLoading || !scrapingUrl.trim(),
              ...
              // Error display
              scrapingError && h('div', { 
                className: `p-3 rounded-lg border-2 ${darkMode ? 'bg-red-900 border-red-700 text-red-200' : 'bg-red-50 border-red-200 text-red-800'}` 
              }, `❌ ${scrapingError}`),
              
              // Results display
              scrapingResults && h('div', { 
                className: `p-3 rounded-lg border-2 ${darkMode ? 'bg-green-900 border-green-700' : 'bg-green-50 border-green-200'}` 
              },
                h('div', { className: 'flex items-center justify-between mb-2' },
                  h('span', { className: `font-semibold ${darkMode ? 'text-green-200' : 'text-green-800'}` }, 
                    `✅ Scraped Successfully ${scrapingResults.cached ? '(Cached)' : '(Fresh)'}`
                  ),
                  h('div', { className: 'flex gap-2' },
                    h('span', { className: `text-xs px-2 py-1 rounded ${darkMode ? 'bg-green-800 text-green-200' : 'bg-green-200 text-green-800'}` },
                      scrapingResults.aiParsed ? '🧠 AI Parsed' : '🔧 Regex Parsed'
                    ),
                    h('span', { className: `text-xs px-2 py-1 rounded ${darkMode ? 'bg-blue-800 text-blue-200' : 'bg-blue-200 text-blue-800'}` },
                      scrapingResults.method
                    )
                  )
                ),
                h('div', { className: `space-y-2 text-sm ${darkMode ? 'text-green-200' : 'text-green-800'}` },
                  h('div', null, `👤 Username: ${scrapingResults.data.username || 'Not found'}`),
                  h('div', null, `📝 Title: ${scrapingResults.data.title || 'Not found'}`),
                  h('div', null, `⏰ Date/Time: ${scrapingResults.data.datetime || 'Not found'}`),
                  scrapingResults.data.aiConfidence && h('div', null, `🎯 AI Confidence: ${Math.round(scrapingResults.data.aiConfidence * 100)}%`)
                ),
                h('div', { className: 'mt-3 pt-3 border-t border-green-300' },
                  h('div', { className: `text-xs ${darkMode ? 'text-green-300' : 'text-green-700'} mb-2` }, 'Apply to template:'),
                  h('div', { className: 'flex gap-2 flex-wrap' },
                    templateList.filter(t => t.isDynamic).map(template =>
                      h('button', {
                        key: template.id,
                        onClick: () => applyScrapedDataToTemplate(template.id, scrapingResults.data),
                        className: `px-3 py-1 text-xs font-medium rounded ${darkMode ? 'bg-green-700 hover:bg-green-600 text-green-200' : 'bg-green-600 hover:bg-green-700 text-white'}`
                      }, template.name.replace(/^(Friendly|Warning) PM: /, ''))
                    )
                  )
                )
              )
          ...
          // Batch scraping
          h('div', { className: `border rounded-lg p-4 ${borderColor}` },
            h('h4', { className: `font-semibold ${textPrimary} mb-3` }, 'Batch Scraping (Multiple URLs)'),
            h('div', { className: 'space-y-3' },
              h('textarea', {
                placeholder: 'Enter multiple eBay community URLs (one per line)...\n\nExample:\nhttps://community.ebay.com/t5/Selling/...\nhttps://community.ebay.com/t5/Buying/...',
                value: batchUrls,
              ...
              h('button', {
                onClick: batchScrapeEbayPosts,
                disabled: isScrapingLoading || !batchUrls.trim(),
                className: `px-4 py-2 text-sm font-medium rounded ${
                  isScrapingLoading || !batchUrls.trim() 
                    ? 'bg-gray-400 cursor-not-allowed text-white' 
                    : 'bg-purple-600 hover:bg-purple-700 text-white'
                }`
              }, isScrapingLoading ? 'Batch Scraping...' : '🚀 Batch Scrape'),
              ...
              batchResults && h('div', { 
                ...
                h('div', { className: 'font-semibold mb-2' }, 
                  `${batchResults.success ? '✅' : '⚠️'} Batch Results: ${batchResults.successfulScrapes}/${batchResults.totalUrls} successful`
                ),
                h('div', { className: 'space-y-1 text-sm' },
                  batchResults.results.map((result, idx) =>
                    h('div', { 
                      ...
                      h('span', { className: 'font-mono text-xs' }, result.url.substring(0, 40) + '...'),
                      result.success 
                        ? h('span', { className: 'text-green-600' }, `✅ ${result.data.username}`)
                        : h('span', { className: 'text-red-600' }, '❌ Failed')
                    )
                  )
                )
              )
          ...
          // Scraping history
          scrapingHistory.length > 0 && h('div', { className: `border rounded-lg p-4 ${borderColor}` },
            h('h4', { className: `font-semibold ${textPrimary} mb-3` }, 'Recent Scraping History'),
            h('div', { className: 'space-y-2 max-h-60 overflow-y-auto' },
              scrapingHistory.map(entry =>
                h('div', { 
                  key: entry.id, 
                  className: `flex items-center justify-between p-2 rounded text-sm ${darkMode ? 'bg-slate-700 border-slate-600' : 'bg-slate-50 border-slate-200'} border`
                },
                  h('div', { className: 'flex-1' },
                    h('div', { className: `font-medium ${textPrimary}` }, entry.data.username || 'Unknown'),
                    h('div', { className: `text-xs ${textSecondary}` }, entry.data.title || 'No title'),
                    h('div', { className: `text-xs ${textSecondary}` }, new Date(entry.timestamp).toLocaleString())
                  ),
                  h('div', { className: 'flex gap-1' },
                    h('span', { className: `text-xs px-2 py-1 rounded ${entry.cached ? 'bg-blue-200 text-blue-800' : 'bg-green-200 text-green-800'}` },
                      entry.cached ? '📋 Cached' : '🔥 Fresh'
                    ),
                    entry.aiParsed && h('span', { className: 'text-xs px-2 py-1 rounded bg-purple-200 text-purple-800' }, '🧠 AI'),
                    h('button', {
                      onClick: () => {
                        const templateOptions = templateList.filter(t => t.isDynamic).map(t => t.name).join('\n');
                        const templateId = prompt(`Apply to which template?\n\nAvailable templates:\n${templateOptions}\n\nEnter template ID:`);
                        if (templateId) {
                          applyScrapedDataToTemplate(templateId, entry.data);
                        }
                      },
                      className: `text-xs px-2 py-1 rounded ${darkMode ? 'bg-slate-600 hover:bg-slate-500 text-slate-200' : 'bg-slate-200 hover:bg-slate-300 text-slate-700'}`
                    }, 'Apply')
                  )
                )
              )
      ...
      h('div', { className: `rounded-lg shadow p-4 text-center text-sm ${cardBg} ${textSecondary} border ${borderColor}` },
        ...
        h('p', { className: 'mt-2 text-xs' }, 'Version 2.2.0')
      )
    )
  );
};
